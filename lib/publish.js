...
function publish (uri, params, cb) {
  ...
  var client = this
  var shasum = sha.getSync(body.path)
  var sink = concat(function (tarbuffer) {
    putFirst.call(client, uri, metadata, tarbuffer, access, auth, shasum, cb)
  })
  sink.on('error', cb)
  body.pipe(sink)
}

function putFirst (registry, data, tarbuffer, access, auth, shasum, cb) {
